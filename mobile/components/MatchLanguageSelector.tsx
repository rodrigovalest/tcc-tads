...
const MatchLanguageSelector = ({ selected, onSelect }: MatchLanguageSelectorProps) => {
  ...
  const getEmojiForLanguage = (value: MatchLanguage): string => {
    const found = SUPPORTED_LANGUAGES.find(l => l.code.toLowerCase().startsWith(`${value.toLowerCase()}-`));
    if (found?.emoji) return found.emoji;
    const fallback: Record<MatchLanguage, string> = { en: "🇬🇧", pt: "🇧🇷", es: "🇪🇸" };
        ...
        {filtered.map(item => {
          ...
          return (
            <TouchableOpacity
              ...
              className={`flex-row items-center justify-between p-4 rounded-lg mb-2 ${isSelected ? 'bg-appLightGrey' : 'bg-white'} border border-gray-200`}
              testID={`match-language-option-${item.value}`}
            >
              <View className="flex-row items-center">
                <Text className="text-xl mr-3">{getEmojiForLanguage(item.value)}</Text>
