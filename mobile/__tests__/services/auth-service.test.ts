...
const MockFormDataBuilder = FormDataBuilder as jest.MockedClass<
  typeof FormDataBuilder
>;
const mockFileUploadService = FileUploadService as jest.Mocked<
...
describe("AuthService", () => {
  ...
  describe("register", () => {
    ...
    beforeEach(() => {
      ...
      mockFileUploadService.createPhotoFile.mockReturnValue(
        mockPhotoFile as any
      );
    });
    ...
    it("should register with photo successfully", async () => {
      ...
      expect(mockFileUploadService.createPhotoFile).toHaveBeenCalledWith(
        mockRegisterRequest.photo
      );
    ...
    it("should handle FormData creation for register with photo", async () => {
      ...
      expect(mockFormDataBuilder.append).toHaveBeenCalledWith(
        "username",
      ...
      expect(mockFormDataBuilder.append).toHaveBeenCalledWith(
        "nationality",
        mockRegisterRequest.nationality
      );
      expect(mockFormDataBuilder.append).toHaveBeenCalledWith(
        "personalDescription",
        mockRegisterRequest.personalDescription
      );
      expect(mockFormDataBuilder.append).toHaveBeenCalledWith(
        "photo",
        mockPhotoFile
      );
      expect(mockFormDataBuilder.appendArray).toHaveBeenCalledWith(
        "languages",
        mockRegisterRequest.languages
      );
      expect(mockFormDataBuilder.appendArray).toHaveBeenCalledWith(
        "interestTopics",
        mockRegisterRequest.interestTopics
      );
    });
  });
});
