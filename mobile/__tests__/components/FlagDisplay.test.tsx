...
describe("FlagDisplay", () => {
  it("should render flag when countryCode is provided and supported", () => {
    const { UNSAFE_root } = render(<FlagDisplay countryCode="br" size={24} />);

    expect(UNSAFE_root).toBeTruthy();
  });

  it("should render with different sizes", () => {
    const { UNSAFE_root } = render(<FlagDisplay countryCode="gb" size={32} />);
  ...
  it("should render with Spanish flag", () => {
    const { UNSAFE_root } = render(<FlagDisplay countryCode="es" size={24} />);

    expect(UNSAFE_root).toBeTruthy();
  });

  it("should handle empty country code", () => {
    const { getByText } = render(<FlagDisplay countryCode="" size={24} />);
