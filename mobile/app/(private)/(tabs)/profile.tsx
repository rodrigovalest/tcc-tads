...
export default function Profile() {
  ...
  const { data: fullUser, isLoading, error, refetch } = useQuery({
  ...
  if (error) {
    return (
      <SafeAreaView className='w-full h-full bg-appBgWhite flex items-center justify-center px-6'>
        <MaterialCommunityIcons name="alert-circle-outline" size={48} color={COLORS.appRed} />
        ...
        <TouchableOpacity 
          ...
          onPress={() => refetch()}
        >
          <Text className='text-white font-nunito-medium'>{t('common.tryAgain')}</Text>
        </TouchableOpacity>
      </SafeAreaView>
    );
  }

  const countryData = fullUser?.nationality ? getCountryData(fullUser.nationality) : null;
      ...
      >
        ...
        <View className='items-center mb-6'>
          <View className='relative'>
            ...
            {/* Status indicator */}
            <View className={`absolute bottom-2 right-2 w-6 h-6 rounded-full border-2 border-white ${fullUser?.isActive ? 'bg-green-500' : 'bg-gray-400'}`} />
          </View>
            ...
            <View>
              {fullUser.languages.map((language, index) => {
                ...
                return (
                  <View 
                    key={language.id} 
                    className={`flex-row items-center justify-between p-3 bg-appLightGrey rounded-lg ${!isLast ? 'mb-3' : ''}`}
                  >
                    <View className='flex-row items-center flex-1'>
                      <Text className='text-2xl mr-3'>{languageData?.flag || '🌐'}</Text>
                      <View>
                    ...
                    <View className='flex-row'>
                      {[...Array(5)].map((_, index) => (
                        <Ionicons
                          key={index}
                          name={index < language.fluencyLevel ? "star" : "star-outline"}
                          size={16}
                          color={index < language.fluencyLevel ? COLORS.appYellow : COLORS.appMediumGrey}
                        />
                      ))}
                    </View>
                  </View>
          ...
          <View className='mb-6 bg-white rounded-2xl p-5 border border-appLightGrey'>
            ...
            <View className='flex-row flex-wrap'>
              {fullUser.interestTopics.map((topic) => (
                <View key={topic.id} className='bg-appDarkGrey px-4 py-2 rounded-full mr-2 mb-2'>
